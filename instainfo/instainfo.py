...
from .instainfocontext import instainfoContext, RateController
...
def _get_config_dir() -> str:
    if platform.system() == "Windows":
        # on Windows, use %LOCALAPPDATA%\instainfo
        localappdata = os.getenv("LOCALAPPDATA")
        if localappdata is not None:
            return os.path.join(localappdata, "instainfo")
        # legacy fallback - store in temp dir if %LOCALAPPDATA% is not set
        return os.path.join(tempfile.gettempdir(), ".instainfo-" + getpass.getuser())
    # on Unix, use ~/.config/instainfo
    return os.path.join(os.getenv("XDG_CONFIG_HOME", os.path.expanduser("~/.config")), "instainfo")
...
def get_legacy_session_filename(username: str) -> str:
    ...
    dirname = tempfile.gettempdir() + "/" + ".instainfo-" + getpass.getuser()
...
def _requires_login(func: Callable) -> Callable:
    ...
    @wraps(func)
    def call(instainfo, *args, **kwargs):
        if not instainfo.context.is_logged_in:
            raise LoginRequiredException("--login=USERNAME required.")
        return func(instainfo, *args, **kwargs)
    return call
...
def _retry_on_connection_error(func: Callable) -> Callable:
    ...
    def call(instainfo, *args, **kwargs):
        try:
            ...
            if (kwargs.get('_attempt') or 1) == instainfo.context.max_connection_attempts:
                raise ConnectionException(error_string) from None
            instainfo.context.error(error_string + " [retrying; skip with ^C]", repeat_at_end=False)
            try:
                if kwargs.get('_attempt'):
                    kwargs['_attempt'] += 1
                else:
                    kwargs['_attempt'] = 2
                instainfo.context.do_sleep()
                return call(instainfo, *args, **kwargs)
            except KeyboardInterrupt:
                instainfo.context.error("[skipped by user]", repeat_at_end=False)
                raise ConnectionException(error_string) from None
    return call
...
class instainfo:
    ...
    .. attribute:: context
       ...
       The associated :class:`instainfoContext` with low-level communication functions and logging.
    """
    ...
    def __init__(self,
                 ...
                 rate_controller: Optional[Callable[[instainfoContext], RateController]] = None,
        ...
        self.context = instainfoContext(sleep, quiet, user_agent, max_connection_attempts,
    ...
    def anonymous_copy(self):
        """Yield an anonymous, otherwise equally-configured copy of an instainfo instance; Then copy its error log."""
        new_loader = instainfo(
    ...
    def login(self, user: str, passwd: str) -> None:
        ...
        :raises TwoFactorAuthRequiredException: First step of 2FA login done, now call
           :meth:`instainfo.two_factor_login`."""
        self.context.login(user, passwd)

    def two_factor_login(self, two_factor_code) -> None:
        """Second step of login if 2FA is enabled.
        Not meant to be used directly, use :meth:`instainfo.two_factor_login`.
        ...
        with resumable_iteration(
            ...
            for number, post in enumerate(posts, start=start_index + 1):
                ...
                if post_filter is not None:
                    try:
                        if not post_filter(post):
                            ...
                            continue
                    except (instainfoException, KeyError, TypeError) as err:
                ...
                with self.context.error_catcher("Download {} of {}".format(post, target)):
                    ...
                    # same Profile.
                    # Observed in issue #225: https://github.com/instainfo/instainfo/issues/225
                    post_changed = False
                    while True:
                        try:
    ...
    def download_profiles(self, profiles: Set[Profile],
        ...
        :param raise_errors:
           ...
           catched and printed with :meth:`instainfoContext.error_catcher`.
    ...
    def download_profile(self, profile_name: Union[str, Profile],
        ...
        .. deprecated:: 4.1
           Use :meth:`instainfo.download_profiles`.
        """
