...
class instainfoContext:
    """Class providing methods for (error) logging and low-level communication with Instagram.

    It is not thought to be instantiated directly, rather :class:`instainfo` instances maintain a context
    object.
    ...
    def __init__(self, sleep: bool = True, quiet: bool = False, user_agent: Optional[str] = None,
        ...
        # error log, filled with error() and printed at the end of instainfo.main()
        ...
        # Can be set to True for testing, disables supression of instainfoContext._error_catcher
    ...
    def is_logged_in(self) -> bool:
        """True, if this instainfo instance is logged in."""
    ...
    def error_catcher(self, extra_info: Optional[str] = None):
        """
        Context manager to catch, print and record instainfoExceptions.
    ...
    def save_session_to_file(self, sessionfile):
        """Not meant to be used directly, use :meth:`instainfo.save_session_to_file`."""
    ...
    def load_session_from_file(self, username, sessionfile):
        """Not meant to be used directly, use :meth:`instainfo.load_session_from_file`."""
        session = requests.Session()
    ...
    def test_login(self) -> Optional[str]:
        """Not meant to be used directly, use :meth:`instainfo.test_login`."""
    ...
    def login(self, user, passwd):
        """Not meant to be used directly, use :meth:`instainfo.login`.
        ...
        if not resp_json['authenticated']:
            if resp_json['user']:
                ...
                # triggers re-asking of password in instainfo.interactive_login(), which makes no sense if the
                # username is invalid.
...
class RateController:
    ...
    It can be overridden to change instainfo's behavior regarding rate limits, for example to raise a custom
    exception when the rate limit is hit::

       import instainfo

       class MyRateController(instainfo.RateController):
           def sleep(self, secs):
               raise MyCustomException()

       L = instainfo.instainfo(rate_controller=lambda ctx: MyRateController(ctx))
    """

    def __init__(self, context: instainfoContext):
    ...
    def handle_429(self, query_type: str) -> None:
        ...
        text_for_429 = ("Instagram responded with HTTP error \"429 - Too Many Requests\". Please do not run multiple "
                        "instances of instainfo in parallel or within short sequence. Also, do not use any Instagram "
                        "App while instainfo is running.")
