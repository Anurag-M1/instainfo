...
from .instainfocontext import instainfoContext
...
class Post:
    ...
    Created by methods :meth:`Profile.get_posts`, :meth:`instainfo.get_hashtag_posts`,
    :meth:`instainfo.get_feed_posts` and :meth:`Profile.get_saved_posts`, which return iterators of Posts::
    ...
    :param context: :attr:`instainfo.context` used for additional queries if neccessary..
    ...
    def __init__(self, context: instainfoContext, node: Dict[str, Any],
    ...
    @classmethod
    def from_shortcode(cls, context: instainfoContext, shortcode: str):
    ...
    @classmethod
    def from_mediaid(cls, context: instainfoContext, mediaid: int):
        """Create a post object from a given mediaid"""
        return cls.from_shortcode(context, Post.mediaid_to_shortcode(mediaid))

    @classmethod
    def from_iphone_struct(cls, context: instainfoContext, media: Dict[str, Any]):
    ...
    def url(self) -> str:
        ...
        if self.typename == "GraphImage" and self._context.iphone_support and self._context.is_logged_in:
            try:
                ...
                return url
            except (instainfoException, KeyError, IndexError) as err:
...
class Profile:
    ...
    @classmethod
    def from_username(cls, context: instainfoContext, username: str):
        """Create a Profile instance from a given username, raise exception if it does not exist.

        See also :meth:`instainfo.check_profile_id`.

        :param context: :attr:`instainfo.context`
    ...
    @classmethod
    def from_id(cls, context: instainfoContext, profile_id: int):
    ...
    @classmethod
    def own_profile(cls, context: instainfoContext):
    ...
    def has_viewable_story(self) -> bool:
        ...
        has at least one story which can be viewed using the associated :class:`instainfoContext`,
    ...
    def profile_pic_url(self) -> str:
        ...
        if self._context.iphone_support and self._context.is_logged_in:
            ...
            except (instainfoException, KeyError) as err:
...
class StoryItem:
    ...
    :param context: :class:`instainfoContext` instance used for additional queries if necessary.
    ...
    def __init__(self, context: instainfoContext, node: Dict[str, Any], owner_profile: Optional[Profile] = None):
        self._context = context
        self._node = node
        self._owner_profile = owner_profile
        self._iphone_struct_ = None
...
class Story:
    ...
    :class:`StoryItem` nodes. Stories are returned by :meth:`instainfo.get_stories`.

    With a logged-in :class:`instainfo` instance `L`, you may download all your visible user stories with::
...
class Highlight(Story):
    ...
    :class:`StoryItem` nodes. Highlights are returned by :meth:`instainfo.get_highlights`.

    With a logged-in :class:`instainfo` instance `L`, you may download all highlights of a :class:`Profile` instance
    USER with::
    ...
    def __init__(self, context: instainfoContext, node: Dict[str, Any], owner: Optional[Profile] = None):
...
class Hashtag:
    ...
    @classmethod
    def from_name(cls, context: instainfoContext, name: str):
        """
...
class TopSearchResults:
    ...
    :param context: :attr:`instainfo.context` used to send the query for the search.
    :param searchstring: String to search for with Instagram's "top search".
    """

    def __init__(self, context: instainfoContext, searchstring: str):
        self._context = context
...
def get_json_structure(structure: JsonExportable) -> dict:
    """Returns instainfo JSON structure for a :class:`Post`, :class:`Profile`, :class:`StoryItem`, :class:`Hashtag`
    ...
    return {
        ...
        'instainfo': {'version': __version__, 'node_type': structure.__class__.__name__}
    }
...
def load_structure(context: instainfoContext, json_structure: dict) -> JsonExportable:
    """Loads a :class:`Post`, :class:`Profile`, :class:`StoryItem`, :class:`Hashtag` or :class:`FrozenNodeIterator` from
    a json structure.

    :param context: :attr:`instainfo.context` linked to the new object, used for additional queries if neccessary.
    :param json_structure: instainfo JSON structure

    .. versionadded:: 4.8
    """
    if 'node' in json_structure and 'instainfo' in json_structure and \
            'node_type' in json_structure['instainfo']:
        node_type = json_structure['instainfo']['node_type']
        if node_type == "Post":
        ...
        # Post JSON created with instainfo v3
        return Post.from_shortcode(context, json_structure['shortcode'])
    raise InvalidArgumentException("Passed json structure is not an instainfo JSON")


def load_structure_from_file(context: instainfoContext, filename: str) -> JsonExportable:
    """Loads a :class:`Post`, :class:`Profile`, :class:`StoryItem`, :class:`Hashtag` or :class:`FrozenNodeIterator` from
