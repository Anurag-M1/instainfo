"""Download & View Private Profiles from Instagram."""
...
from . import (AbortDownloadException, BadCredentialsException, instainfo, instainfoException,
...
from .instainfo import (get_default_session_filename, get_default_stamps_filename)
from .instainfocontext import default_user_agent
...
def usage_string():
    ...
    argv0 = "instainfo" if argv0 == "__main__.py" else argv0
    return """
{0} [--comments] [--geotags]
...
def _main(instainfo: instainfo, targetlist: List[str],
    ...
    if post_filter_str is not None:
        ...
        instainfo.context.log('Only download posts with property "{}".'.format(post_filter_str))
    ...
    if storyitem_filter_str is not None:
        ...
        instainfo.context.log('Only download storyitems with property "{}".'.format(storyitem_filter_str))
    latest_stamps = None
    if latest_stamps_file is not None:
        latest_stamps = LatestStamps(latest_stamps_file)
        instainfo.context.log(f"Using latest stamps from {latest_stamps_file}.")
    # Login, if desired
    if username is not None:
        if not re.match(r"^[A-Za-z0-9._]+$", username):
            instainfo.context.error("Warning: Parameter \"{}\" for --login is not a valid username.".format(username))
        try:
            instainfo.load_session_from_file(username, sessionfile)
        except FileNotFoundError as err:
            if sessionfile is not None:
                print(err, file=sys.stderr)
            instainfo.context.log("Session file does not exist yet - Logging in.")
        if not instainfo.context.is_logged_in or username != instainfo.test_login():
            if password is not None:
                try:
                    instainfo.login(username, password)
                except TwoFactorAuthRequiredException:
                    while True:
                        try:
                            code = input("Enter 2FA verification code: ")
                            instainfo.two_factor_login(code)
                            break
                        except BadCredentialsException as err:
                            print(err, file=sys.stderr)
                            pass
            else:
                instainfo.interactive_login(username)
        instainfo.context.log("Logged in as %s." % username)
    # since 4.2.9 login is required for geotags
    if instainfo.download_geotags and not instainfo.context.is_logged_in:
        instainfo.context.error("Warning: Use --login to download geotags of posts.")
    ...
    try:
        ...
        for target in targetlist:
            if (target.endswith('.json') or target.endswith('.json.xz')) and os.path.isfile(target):
                with instainfo.context.error_catcher(target):
                    structure = load_structure_from_file(instainfo.context, target)
                    if isinstance(structure, Post):
                        if post_filter is not None and not post_filter(structure):
                            instainfo.context.log("<{} ({}) skipped>".format(structure, target), flush=True)
                            continue
                        instainfo.context.log("Downloading {} ({})".format(structure, target))
                        instainfo.download_post(structure, os.path.dirname(target))
                    elif isinstance(structure, StoryItem):
                        if storyitem_filter is not None and not storyitem_filter(structure):
                            instainfo.context.log("<{} ({}) skipped>".format(structure, target), flush=True)
                            continue
                        instainfo.context.log("Attempting to download {} ({})".format(structure, target))
                        instainfo.download_storyitem(structure, os.path.dirname(target))
            ...
            with instainfo.context.error_catcher(target):
                if re.match(r"^@[A-Za-z0-9._]+$", target):
                    instainfo.context.log("Retrieving followees of %s..." % target[1:])
                    profile = Profile.from_username(instainfo.context, target[1:])
                    for followee in profile.get_followees():
                        instainfo.save_profile_id(followee)
                        profiles.add(followee)
                elif re.match(r"^#\w+$", target):
                    instainfo.download_hashtag(hashtag=target[1:], max_count=max_count, fast_update=fast_update,
                                                 post_filter=post_filter,
                                                 profile_pic=download_profile_pic, posts=download_posts)
                elif re.match(r"^-[A-Za-z0-9-_]+$", target):
                    instainfo.download_post(Post.from_shortcode(instainfo.context, target[1:]), target)
                elif re.match(r"^%[0-9]+$", target):
                    instainfo.download_location(location=target[1:], max_count=max_count, fast_update=fast_update,
                                                  post_filter=post_filter)
                elif target == ":feed":
                    instainfo.download_feed_posts(fast_update=fast_update, max_count=max_count,
                                                    post_filter=post_filter)
                elif target == ":stories":
                    instainfo.download_stories(fast_update=fast_update, storyitem_filter=storyitem_filter)
                elif target == ":saved":
                    instainfo.download_saved_posts(fast_update=fast_update, max_count=max_count,
                                                     post_filter=post_filter)
                elif re.match(r"^[A-Za-z0-9._]+$", target):
                    try:
                        profile = instainfo.check_profile_id(target, latest_stamps)
                        if instainfo.context.is_logged_in and profile.has_blocked_viewer:
                            if download_profile_pic or ((download_posts or download_tagged or download_igtv)
                                ...
                                instainfo.context.error("{} blocked you.".format(target))
                        else:
                        ...
                        if instainfo.context.is_logged_in and (download_profile_pic or download_posts or
                                                                 download_tagged or download_igtv):
                            instainfo.context.log(err)
                            instainfo.context.log("Trying again anonymously, helps in case you are just blocked.")
                            with instainfo.anonymous_copy() as anonymous_loader:
                                with instainfo.context.error_catcher():
                                    anonymous_retry_profiles.add(anonymous_loader.check_profile_id(target,
                                                                                                   latest_stamps))
                                    instainfo.context.error("Warning: {} will be downloaded anonymously (\"{}\")."
                                                              .format(target, err))
                        else:
                            raise
                else:
        ...
        if len(profiles) > 1:
            instainfo.context.log("Downloading {} profiles: {}".format(len(profiles),
                                                                         ' '.join([p.username for p in profiles])))
        if instainfo.context.iphone_support and profiles and (download_profile_pic or download_posts) and \
           not instainfo.context.is_logged_in:
            instainfo.context.log("Hint: Use --login to download higher-quality versions of pictures.")
        instainfo.download_profiles(profiles,
        ...
        if anonymous_retry_profiles:
            instainfo.context.log("Downloading anonymously: {}"
    ...
    if instainfo.context.is_logged_in:
        instainfo.save_session_to_file(sessionfile)
    # User might be confused if instainfo does nothing
    if not targetlist:
        if instainfo.context.is_logged_in:
            # instainfo did at least save a session file
            instainfo.context.log("No targets were specified, thus nothing has been downloaded.")
        else:
            # instainfo did not do anything
            instainfo.context.log("usage:" + usage_string())
...
def main():
    parser = ArgumentParser(description=__doc__, add_help=False, usage=usage_string(),
                            epilog="The complete documentation can be found at "
                                   "https://instainfo.github.io/.",
                            fromfile_prefix_chars='+')

    g_targets = parser.add_argument_group("What to Download",
                                          "Specify a list of targets. For each of these, instainfo creates a folder "
                                          "and downloads all posts. The following targets are supported:")
    g_targets.add_argument('profile', nargs='*',
                           help="Download profile. If an already-downloaded profile has been renamed, instainfo "
    ...
    g_cond.add_argument('-F', '--fast-update', action='store_true',
                        help='For each target, stop when encountering the first already-downloaded picture. This '
                             'flag is recommended when you use instainfo to update your personal Instagram archive.')
    ...
    g_cond.add_argument('--post-filter', '--only-if', metavar='filter',
                        help='Expression that, if given, must evaluate to True for each post to be downloaded. Must be '
                             ...
                             'instainfo.Post attributes. Example: --post-filter=viewer_has_liked.')
    g_cond.add_argument('--storyitem-filter', metavar='filter',
                        help='Expression that, if given, must evaluate to True for each storyitem to be downloaded. '
                             ...
                             'instainfo.StoryItem attributes.')
    ...
    g_login = parser.add_argument_group('Login (Download Private Profiles)',
                                        'instainfo can login to Instagram. This allows downloading private profiles. '
                                        'To login, pass the --login option. Your session cookie (not your password!) '
                                        'will be saved to a local file to be reused next time you want instainfo '
                                        'to login.')
    ...
    g_how.add_argument('--abort-on', type=http_status_code_list, metavar="STATUS_CODES",
                       help='Comma-separated list of HTTP status codes that cause instainfo to abort, bypassing all '
                            'retry logic.')
    ...
    g_misc.add_argument('-q', '--quiet', action='store_true',
                        help='Disable user interaction, i.e. do not print messages (except errors) and fail '
                             'if login credentials are needed but not given. This makes instainfo suitable as a '
                             'cron job.')
    ...
    try:
        ...
        loader = instainfo(sleep=not args.no_sleep, quiet=args.quiet, user_agent=args.user_agent,
        ...
        loader.close()
    except instainfoException as err:
