...
ratecontroller = None  # type: Optional[instainfo.RateController]


class TestinstainfoAnonymously(unittest.TestCase):

    def setUp(self):
        self.dir = tempfile.mkdtemp()
        print("Testing in {}".format(self.dir))
        os.chdir(self.dir)
        self.L = instainfo.instainfo(download_geotags=True,
    ...
    def test_public_profile_paging(self):
        self.post_paging_test(instainfo.Profile.from_username(self.L.context, PUBLIC_PROFILE).get_posts())
    ...
    def test_hashtag_paging(self):
        for count, post in enumerate(instainfo.Hashtag.from_name(self.L.context, HASHTAG).get_all_posts()):
            print(post)
    ...
    def test_get_id_by_username(self):
        self.assertEqual(PUBLIC_PROFILE_ID,
                         instainfo.Profile.from_username(self.L.context, PUBLIC_PROFILE).userid)

    def test_get_username_by_id_private(self):
        self.assertEqual(PRIVATE_PROFILE.lower(),
                         instainfo.Profile.from_id(self.L.context, PRIVATE_PROFILE_ID).username)

    def test_get_username_by_id_public(self):
        self.assertEqual(PUBLIC_PROFILE.lower(),
                         instainfo.Profile.from_id(self.L.context, PUBLIC_PROFILE_ID).username)

    def test_get_username_by_id_empty(self):
        self.assertEqual(EMPTY_PROFILE.lower(),
                         instainfo.Profile.from_id(self.L.context, EMPTY_PROFILE_ID).username)

    def test_get_username_by_name_empty(self):
        self.assertEqual(EMPTY_PROFILE_ID,
                         instainfo.Profile.from_username(self.L.context, EMPTY_PROFILE).userid)

    def test_post_from_mediaid(self):
        for post in instainfo.Profile.from_username(self.L.context, PUBLIC_PROFILE).get_posts():
            post2 = instainfo.Post.from_mediaid(self.L.context, post.mediaid)
            self.assertEqual(post, post2)
            break

    def test_public_profile_tagged_paging(self):
        for post in islice(instainfo.Profile.from_username(self.L.context, PUBLIC_PROFILE).get_tagged_posts(),
                           PAGING_MAX_COUNT):
            print(post)

    def test_public_profile_igtv(self):
        for post in islice(instainfo.Profile.from_username(self.L.context, PUBLIC_PROFILE_WITH_IGTV).get_igtv_posts(),
                           PAGING_MAX_COUNT):
            print(post)


class TestinstainfoLoggedIn(TestinstainfoAnonymously):
    ...
    def test_private_profile_paging(self):
        self.post_paging_test(instainfo.Profile.from_username(self.L.context, PRIVATE_PROFILE).get_posts())
    ...
    def test_saved_paging(self):
        for count, post in enumerate(instainfo.Profile.from_username(self.L.context, OWN_USERNAME).get_saved_posts()):
            print(post)
            if count == PAGING_MAX_COUNT:
                break
    ...
    def test_followees_and_stories(self):
        profile = instainfo.Profile.from_username(self.L.context, OWN_USERNAME)
    ...
    def test_get_likes(self):
        for post in instainfo.Profile.from_username(self.L.context, OWN_USERNAME).get_posts():
