...
Here we present code examples that use the :ref:`python-module-instainfo` for
more advanced Instagram downloading or metadata mining than what is possible
with the instainfo command line interface.

The scripts presented here can be downloaded from our source tree:
`instainfo/docs/codesnippets/ <https://github.com/instainfo/instainfo/tree/master/docs/codesnippets>`__
...
See also :class:`Post`, :meth:`instainfo.download_post`.
...
:meth:`Profile.get_followers`, :meth:`instainfo.load_session_from_file`,
...
This script uses instainfo to obtain a list of currently-online Instagram and
...
With instainfo, it is easy to download the few most-liked pictures of a user.
...
The JSON files instainfo saves along with each Post contain all the metadata
...
metadata can be easily post-processed. For example, instainfo's JSON files can
...
However, instainfo tries to do as few metadata requests as possible, so,
depending on how instainfo has been invoked, it may occur that these files do
not contain the complete available metadata structure. Nevertheless, the file
can be loaded into instainfo with :func:`load_structure_from_file` and the
