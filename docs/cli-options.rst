...
instainfo is invoked with::

   $ instainfo [options] target [target ...]
...
Here we explain the additional options that can be given to instainfo to
customize its behavior.  For an
introduction on how to use instainfo, see
...
Specify a list of targets. For each of these, instainfo creates a folder and
...
.. option:: --fast-update, -F
   ...
   This flag is recommended when you use instainfo to update your personal
   Instagram archive.
...
.. option:: --latest-stamps [STAMPSFILE]
   ...
   ``~/.config/instainfo/latest-stamps.ini``, but you can specify an
...
.. option:: --post-filter filter, --only-if filter
   ...
   evaluated to :class:`instainfo.Post` attributes.  Example:
...
.. option:: --storyitem-filter filter
   ...
   downloaded.  Must be a syntactically valid Python expression. Variables are
   evaluated to :class:`instainfo.StoryItem` attributes.
...
instainfo can login to Instagram. This allows downloading private
...
.. option:: --sessionfile SESSIONFILE, -f SESSIONFILE
   ...
   ``~/.config/instainfo/session-YOUR-USERNAME``.
...
.. option:: --resume-prefix prefix
   ...
   For many targets, instainfo is capable of resuming a previously-aborted
...
.. option:: --user-agent USER_AGENT
   ...
   User Agent to use for HTTP requests. Per default, instainfo pretends being
   Chrome/92 on Linux.
...
.. option:: --abort-on STATUS_CODE_LIST
   ...
   Comma-separated list of HTTP status codes that cause instainfo to abort,
   bypassing all retry logic.

   For example, with ``--abort-on=302,400,429``, instainfo will stop if a
...
.. option:: --quiet, -q
   ...
   This is handy for running :ref:`instainfo-as-cronjob`.
