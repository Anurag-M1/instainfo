...
shutil.copy('deploy/windows/instainfo.spec', '.')
...
with open('instainfo/__main__.py', 'r') as f:
    # adjust imports for changed file structure
    regex = re.compile(r'from (?:(\.[^ ]+ )|\.( ))import')
    lines = [regex.sub(r'from instainfo\1\2import', line) for line in f.readlines()]
...
commands = ["pip install pipenv==2023.2.4",
            ...
            "pipenv run pyinstaller --log-level=DEBUG instainfo.spec"]
...
with open('dist/instainfo.exe', 'rb') as f:
    for chunk in iter(lambda: f.read(4096), b''):
        hash_md5.update(chunk)

with open('dist/instainfo.exe.md5', 'w+') as f:
    f.write('{} *instainfo.exe\n'.format(hash_md5.hexdigest()))

# Create ZIP file
shutil.make_archive('instainfo-{}-windows-standalone'.format(os.getenv('VERSION_TAG')), 'zip', 'dist')
